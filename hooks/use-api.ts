...
export function useMerchantProduits(merchantUserId: string) {
  return useQuery({
    ...
    queryFn: () => apiClient.get<ApiResponse<ProduitEcommerceDto[]>>("/merchant/produits", { merchantUserId }),
    enabled: !!merchantUserId,
  })
}
...
export function useAjouterProduitMerchant() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/merchant/produits`, {
        method: "POST",
        headers: {
